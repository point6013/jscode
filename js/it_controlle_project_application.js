...
function IT_project(){
    ...
    if(urlParams.status == "look"){
        $(".icon-database-menu").parent().css("display","none");
    }else {
         $(".icon-database-menu").parent().css("display","show");
    }
    spread.suspendPaint();
    ...
    dataSheet.floatRowTable[0].stateNotSaveRow({sheetIndex:0, rowIndex:rno})
    var colI2 ='A'
    sheet.setValue(rno, 0, "Total")
    sheet.getCell(rno, 0).font('bold normal 13px normal')
    for (var i = ConvertNum("L"); i <= ConvertNum("U"); i++) {
        var strFormula ='';
        var col =Convert26(i);
...
const addDownloadButton = () => {
  downloadDOM = `<a class=\"breadcrumb-elements-item cursor-pointer mr-2 ml-2 mcdPythonButton\"
    onclick=\"tableExportFunction()\"><i class=\"mi-cloud-download icon text-default mr-2\"></i>下载SAP明细</a>
  `;
  if ($('.mi-cloud-download').length === 0) {
    $('.viewTransformation').append(downloadDOM);
  }
};
...
function tableExportFunction(){
    ...
    var downloadSQL = `
     ...
     ZTERM
      ...
      from 
     mcdcapex.app1_apply_if_to_be_part_sap_actual_pay_detail
     where GJAHR =${year_down}`
     console.log(downloadSQL)
    var resDownload = cfs.request.foundation.runComm(downloadSQL);
    var theadData = ['BUKRS',
    'BELNR','BLART','HKONT',
    'BUZEI','SHKZG','PRCTR','GJAHR','MONAT',
    'BLDAT','BUDAT',
    'SGTXT','WRBTR','PSWSL', 'ZUONR','XREF1',
    'XREF2','XREF3',
    'last_update_time','BKTXT',
    'KOART','AUGBL','AUGDT','LIFNR','ZTERM'];
    if (resDownload.res.length !== 0) {
        cfs.export.toCsv("SAP__Detail_Download",resDownload.res,theadData);
