...
//   var proj_year = cfs.request.foundation.queryonevariableinfo('ProjYear').res.resultObj.value;  
 ...
 function before_save(){
    var prodduct_list = `select description_1 from app1_di_product_table_dimension where parent_name in ('Product')`;

    // JS执行查询SQL
    var res = cfs.request.foundation.runComm(prodduct_list);
     debugger;
    var sheet =spread.getSheet(0);
    // sheet.setColumnVisible(1,false);  //隐藏code列
    // var arr1 = sheet.getArray(1, 0, sheet.getRowCount(), 3);
    debugger;
    endLine = sheet.getRowCount();
    //A列为空时C列不可编辑
    for (var i = 1; i <= endLine; i++){
        ...
        var cellValue1 =  sheet.getValue(i, 2) ;
    // debugger;
    if (cellValue && cellValue === "Existing") {
        sheet.getCell(i, 2).backColor("White"); //  区域底色变白
        sheet.getCell(i, 2).locked(true);  //  区域锁定
