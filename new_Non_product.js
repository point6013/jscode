function NonProd(){
    ...
    sheet.setColumnVisible(9,false);  //隐藏Budg Code
   ...
   for (var i = 1; i <= rowCount; i++){
    var cellValue =  sheet.getValue(i, 10) 
    if (cellValue && cellValue === "Plan") {
        sheet.getCell(i, 3).backColor("White"); //  区域底色变白
        sheet.getCell(i, 8).backColor("White"); //  区域底色变白
        sheet.getCell(i, 7).backColor("White"); //  区域底色变白
        sheet.getCell(i, 6).backColor("White"); //  区域底色变白
        sheet.getCell(i,5).backColor("White"); //  区域底色变白
        sheet.getCell(i, 4).backColor("White"); //  区域底色变白

        sheet.getCell(i, 8).locked(true);  //  区域锁定
        sheet.getCell(i, 7).locked(true);  //  区域锁定
        sheet.getCell(i, 6).locked(true);  //  区域锁定
        sheet.getCell(i, 5).locked(true);  //  区域锁定
        sheet.getCell(i, 4).locked(true);  //  区域锁定
        sheet.getCell(i,3).locked(true);  //  区域锁定
...
function setLValue(sheet,rowNum){
    ...
    // 设置B列单元格的值
    sheet.getCell(rowNum,1).cellType(new SmartListCellType(newArr));
}